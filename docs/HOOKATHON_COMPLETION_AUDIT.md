...
| --- | --- | --- |
| Design a memorable use case | Ready | `docs/HOOKATHON_USECASE_ULTRAMAR_PORT_OF_CALL.md` defines Ultramar Port of Call: Abloh-inspired private-market discovery plus passport-gated v4 investment ports. |
...
| Make it Abloh-esque | Ready | The product loop is travel feed -> local guide room -> translated diligence -> passport stamp -> capital window -> return ticket. |
...
| Avoid a generic permissioned pool | Ready | The hook replaces AMM price discovery with a sandbox signed window settlement schedule, rejects public LP modification, and binds signed passports to `CapitalWindowRouter`. The signed demo term is explicit: `4.5M USD` sandbox pre-money, `4.5M restricted LCX sandbox units`, signed FX snapshot, `1,000 demo USDC` step, `10%` tranche premium, and `1,500 demo USDC -> 1,454.54 sandbox restricted LCX` at `1.0312 demo USDC/restricted LCX` effective. |
...
| Provide public-link preflight | Ready externally | `corepack yarn hookathon:links:check` verifies the Tally form, public GitHub branch, raw root README Hookathon index, raw Tally copy, production demo, production deck, pricing markers, judge packet links, video asset, captions asset, release page, Uniswap v4 whitepaper, public Atrium course page, and v4 docs sources, with retries for transient network/5xx responses, then writes `artifacts/hookathon/public-links-latest.md`. |
| Provide public render QA | Ready externally | `corepack yarn hookathon:public:render:qa` opens the production demo and deck with Playwright in desktop/mobile viewports, verifies key content including the pricing slide markers (`Pre-money and FX`, `Snapshot, then fixed`, `Fixed window`, `Floating policy`, `FX snapshot locked`, `Disclosed step schedule`, `1.0312`, `effective 1.0312`) and judge packet markers (`Base Sepolia proof`, `Winning scorecard`, `Source branch`), visible media, screenshots, and page-level horizontal overflow, then writes `artifacts/hookathon/public-render-qa-latest.md`. |
...
| Public GitHub branch | Ready externally after push | `gh repo view Diegolden-com/ultramar-private-markets --json visibility,url` returned `visibility: PUBLIC` and `https://github.com/Diegolden-com/ultramar-private-markets` on May 31, 2026. The Tally copy points judges to `https://github.com/Diegolden-com/ultramar-private-markets/tree/codex/landing-wave-route-ui`; the root `README.md` now exposes the Hookathon judge packet with demo, deck, video, Base Sepolia proof, scorecard, fast path, and commands. |
| Public frontend deployment | Ready externally | Production routes were redeployed and verified live on June 11, 2026: `https://ultramar.capital/hookathon/port-of-call` and `https://ultramar.capital/hookathon/port-of-call/deck` returned HTTP 200 after Vercel aliasing. The demo route includes the operating-readiness markers `Admin work becomes underwriting evidence.`, `Current asset coverage`, and `Hook access`; the deck route includes the pricing graph markers `FX snapshot locked`, `Disclosed step schedule`, `Fixed window`, `Floating policy`, and `effective 1.0312`, plus `basePrice = preMoneyUsd / fullyDilutedUnits`; both demo and deck expose judge packet links for video, Base Sepolia proof, scorecard, and source branch. |
...
| Demo video upload | Ready externally | GitHub release `hookathon-port-of-call-demo-2026-05-31` includes refreshed `final-demo-latest.webm`, captions, and public `testnet-dry-run-latest.md` proof uploaded on June 11, 2026. The WebM is 6,301,051 bytes and the VTT is 2,548 bytes. The VTT verifies `Admin work becomes underwriting evidence.`, `Daily close, margin route, current asset coverage`, `testWindowStepCurveQuotesExactPricingExample`, and `1.0312 effective`. Direct video URL: `https://github.com/Diegolden-com/ultramar-private-markets/releases/download/hookathon-port-of-call-demo-2026-05-31/final-demo-latest.webm`. |
