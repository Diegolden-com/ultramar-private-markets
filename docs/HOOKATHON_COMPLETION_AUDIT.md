...
| Provide final submit operator | Ready locally | `corepack yarn hookathon:submission:operator` refreshes public links, Tally field mapping, fill plan, browser session pack, preflight, and readiness, then writes ignored `artifacts/hookathon/final-submit-run-latest.md`. In `--strict` mode it requires personal env vars and a submit-ready session pack. |
