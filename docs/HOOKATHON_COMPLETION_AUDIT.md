...
| Avoid a generic permissioned pool | Ready | The hook replaces AMM price discovery with a sandbox windowed conversion curve, rejects public LP modification, and binds signed passports to `CapitalWindowRouter`. The signed demo term is explicit: `4.5M USD` sandbox pre-money, `4.5M restricted LCX sandbox units`, signed FX snapshot, `1,000 demo USDC` step, `10%` tranche premium, and `1,500 demo USDC -> 1,454.54 sandbox restricted LCX` at `1.0312 demo USDC/restricted LCX` effective. |
