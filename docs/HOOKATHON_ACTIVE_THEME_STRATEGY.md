...
**Ultramar Port of Call is a Uniswap v4 Specialized Market for private operating-business capital windows.**

The hook specializes the market around the constraints of private operating-business capital: eligibility, transfer policy, ticket size, timing, allocation caps, oracle freshness, router provenance, and deterministic exact-input settlement.

## Evidence map

| UHI8 Specialized Markets need | Port of Call proof |
| --- | --- |
| Asset-class-specific liquidity | Private operating-business capital windows have eligibility, timing, ticket, transfer, and issuer-proof constraints that generic public AMMs do not model. |
| Custom pricing and risk parameters | `beforeSwapReturnDelta` replaces open AMM price discovery with a deterministic capital-window conversion curve. |
...
> Generic AMMs are not expressive enough for private operating-business capital. Port of Call uses Uniswap v4 to create a specialized capital-window market: signed passport, exact-input execution, capped windows, fresh issuer proof, blocked generic routing, and custom accounting for deterministic conversion terms.

Then show:

1. Investor travels to Lavanderias CX.
2. Passport stamp creates signed `hookData`.
3. Approved exact-input swap settles `1500.00` USDC -> `1454.54` LCX.
4. Generic router or replay attempt reverts.
5. Public liquidity modification is blocked by tests.
